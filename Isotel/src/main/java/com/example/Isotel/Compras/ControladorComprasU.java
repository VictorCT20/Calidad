...
@Controller
public class ControladorComprasU {
    ...
    public String Mostrar(Model model, @AuthenticationPrincipal UserDetails userDetails)
    {
        ...
        List<Compras> comprasU = new ArrayList<>();
        
        for (int i = 0; i < compras.size(); i++) {
            if(compras.get(i).getUsuario().getUsername().equals(userDetails.getUsername())){
                comprasU.add(compras.get(i));
            }
        }
        
        
        model.addAttribute("compras", comprasU);
        ...
        Usuario usuario = serviceU.BuscarA(userDetails.getUsername());
